    ...
    public class ConfigurationProvider<TSettings> : IConfigurationProvider<TSettings> where TSettings : ISettings, new()
    {
        ...
        private void BuildConfiguration() 
        {
            ...
            var properties = from prop in typeof(TSettings).GetProperties()
                             ...
                             where setting != null
                             where CommonHelper.GetApCustomTypeConverter(prop.PropertyType).CanConvertFrom(typeof(string))
                             where CommonHelper.GetApCustomTypeConverter(prop.PropertyType).IsValid(setting)
                             let value = CommonHelper.GetApCustomTypeConverter(prop.PropertyType).ConvertFromInvariantString(setting)
